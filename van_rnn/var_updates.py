...
def get_Wbar_p_params(h, gamma,v,x,xxT,Sigma_inv,mu,T,d, ud,
                      alpha, tau, inv_var):
    h = h[1:]
    A = np.zeros((d, d+ud+1,d+ud+1))
    rhs = np.zeros((d,d+ud+1,1))
    for i in range(0,d):
        ...
        if np.sum(check) > 0:
            ...
            zrx_term[check] += inv_var[i]*(1/alpha**2)*xxT[check]
            mid_term += zrx_term
...
def Wbar_p_update(h, gamma,v,x,xxT,Sigma_inv,mu,T,d,ud, alpha, tau, inv_var):
    W_covar, W_mu = get_Wbar_p_params(h, gamma,v,x,xxT,
                                      Sigma_inv,mu,T,d, ud,
                                      alpha, tau, inv_var)
    W_bar = sample_weights(W_covar, W_mu, d, ud)
    W,U,b = extract_W_weights(W_bar, d, ud)
