...
def get_elbo(T, d,  c_0, inv_covar_c, h_0, inv_covar_h, 
             Wi, Wf, Wp, Wo, 
             Ui, Uf, Up, Uo, u, 
             Ec, Ecc_diags, Ecc_off_diags, Sigma_c, 
             Ev, Eh, Ehh_diags, Sigma_h, 
    ...
    '''
    ###
    #multiply times 4 when Wi=Wf=Wp input to save time
    elbo_omega2 = elbo_PG2(gi, 'omega_i')
    elbo += elbo_omega2*4
    ###
    '''
    
    elbo += elbo_PG2(gi, 'omega_i')
    elbo += elbo_PG2(gf, 'omega_f')
    elbo += elbo_PG2(gp, 'omega_p')
    elbo += elbo_PG2(go, 'omega_o')
    ...
    elbo += entropy_PG(g_gamma,'gamma')

    '''
    ### multiply times 4 since same input to save time
    ent_omega = entropy_PG(gi, 'i')
    elbo += ent_omega*4
    ###
    '''


    elbo += entropy_PG(gi, 'omega_i')
    elbo += entropy_PG(gf, 'omega_f')
    elbo += entropy_PG(gp, 'omega_p')
    elbo += entropy_PG(go, 'omega_o')
...
u = .4*np.ones((T, ud, 1))
'''
Wi = .1*np.ones((d,d)) #np.random.uniform(-1,1, size=(d,d))
Wf = .2*np.ones((d,d)) #np.random.uniform(-1,1, size=(d,d))
Wp = .3*np.ones((d,d)) #np.random.uniform(-1,1, size=(d,d))
Wo = -.4*np.ones((d,d)) #np.random.uniform(-1,1, size=(d,d))

Ui = .4*np.ones((d,ud)) #np.random.uniform(-1,1, size=(d,ud))
Uf = .3*np.ones((d,ud)) #np.random.uniform(-1,1, size=(d,ud))
Up = -.2*np.ones((d,ud)) #np.random.uniform(-1,1, size=(d,ud))
Uo = .1*np.ones((d,ud)) #np.random.uniform(-1,1, size=(d,ud))

bi = -.3*np.ones((d,1)) #np.random.uniform(-1,1, size=(d,1))
bf = .5*np.ones((d,1)) #np.random.uniform(-1,1, size=(d,1))
bp = .7*np.ones((d,1)) #np.random.uniform(-1,1, size=(d,1))
bo = .6*np.ones((d,1)) #np.random.uniform(-1,1, size=(d,1))
'''
...
for k in range(0,1):
    ...
    #update q_h:
    Lambda_h, Lambda_h_m = update_qh(T, d, h_0, inv_covar_h, 
                                     Wi, Wf, Wp,  Wo, u, 
                                     Ui, Uf, Up, Uo, Ev, 
                                     Eomega_i, Eomega_f, 
                                     Eomega_p, Eomega_o, 
