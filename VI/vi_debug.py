...
T = stop-1
u = np.zeros((T,1,1))#data[:stop-1].reshape(T,ud,1)
y = data[1:stop].reshape(T,yd,1)

T_test = T_full-T
u_test = np.zeros((T,1,1))#data[stop-1:-1].reshape(T_test,ud,1)
...
W_bar_i_old = np.ones((d, d+0+1))*np.inf
W_bar_f_old = np.ones((d, d+0+1))*np.inf
W_bar_p_old = np.ones((d, d+0+1))*np.inf
W_bar_o_old = np.ones((d, d+0+1))*np.inf
...
#for k in range(0,10):
    ...
    W_bar_i = update_W_bar_star(T, d, 0, Ex, Exx, Eomega_i, Ezi)
    ...
    #Update Wf
    W_bar_f = update_W_bar_star(T, d, 0, Ex, Exx, Eomega_f, Ezf)
    ...
    #Update Wp
    W_bar_p = update_W_bar_star(T, d, 0, Ex, Exx, 2*Eomega_p, Ezp)
    ...
    #Update Wo
    W_bar_o = update_W_bar_star(T, d, 0, Ex, Exx, Eomega_o, Ezo)
...
for j in range(0,T_new):
    ...
    y_tr_gen, ct, ht, _,_,_,_,_ = generate(1,d, yd, input, 
                                         ct.reshape(d), Sig_c.reshape(d), 
                                         ht.reshape(d), Sig_h.reshape(d), 
                                         Sigma_y,
                                         Wy, Wi, Wf, Wp, Wo, 
                                         Ui, Uf, Up, Uo, 
                                         by.reshape(yd), bi.reshape(d), 
