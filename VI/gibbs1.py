...
def update_W_bar_y(T, d, yd, y, Ex, Exx):
    ...
    #sumyExT @ np.linalg.inv(sumExx)
    return sumExx, sumyExT 

def update_Sigma_y_old(T, yd, y, W_bar, Ex, Exx):
    A = np.zeros((T, yd, yd))
...
def update_Sigma_y(T, d, yd, y, Ex, Exx):

    yy  = (y[...,None]*y[:,None,:]).reshape(T,yd,yd)
    sum_yy = np.sum(yy, axis=0)
    
    xyT = (Ex[...,None]*y[:,None,:]).reshape(T,d+1,yd)
    sum_xyT = np.sum(xyT, axis=0)
    #sum_yxT = sum_xyT.transpose(0,2,1)
    sum_xx = np.sum(Exx, axis=0)
    
    return sum_yy, sum_xyT, sum_xx
...
def update_W_bar_star(T, d, ud, Ex, Exx, Eomega_star, Ez_star):
    ...
    #np.linalg.solve(A,rhs).reshape(d,d+ud+1)
    return A, rhs
...
v = Ev.reshape(T,d,1) 
zi = Ezi.reshape(T,d,1) 
zf = Ezf.reshape(T,d,1) 
zp = Ezp.reshape(T,d,1) 
zo = Ezo.reshape(T,d,1) 
...
def update_Sigma_H_old(T, d, Eh, Ehh_diags, Ezo, Ev):
    Ehh_u = Ehh_diags.reshape(T,d)
    Eh_u = Eh.reshape(T,d)
    Ezo_u = Ezo.reshape(T,d)
    Ev_u = Ev.reshape(T,d)
...
def update_Sigma_C_old(T,d, c_0, Ecc_diags, Ecc_off_diags, Ec, Ezi, Ezf, Ezp):
    c0 = c_0.reshape(1,d)
    Ec_u = Ec.reshape(T,d)
    Ecc_u = Ecc_diags.reshape(T,d)
    Ecc_min_u = np.concatenate((c0**2, Ecc_u[:-1,:]), axis=0)
...
Sig_h = update_Sigma_H_old(T, d, Eh, Ehh_diags, Ezo, Ev)
inv_covar_h = 1/Sig_h
    
Sig_c = update_Sigma_C_old(T,d, c_0, Ecc_diags, 
                       Ecc_off_diags, Ec, Ezi, Ezf, Ezp)
inv_covar_c = 1/Sig_c

W_bar_y = np.concatenate((Wy, by), axis =1)
Ex_y, Exx_y = get_Ex_Exx_No_U(T, d, Eh, Ehh) 
Sigma_y = update_Sigma_y_old(T, yd, y, W_bar_y, Ex_y, Exx_y)
...
for k in range(0,150):
    ...
    for n in range(0,N):
        ...
        if n > N_burn:
            Ec_add += Ec
            Eh_add += Eh
             #Suff stats for weight updates:
            h_min, hh_min = get_Ehmin_Ehhmin(T, d, h, hh, h_0)
            x, xx = get_Ex_Exx(T, d, ud, h, hh, u)

        
            A, rhs = update_W_bar_star(T, d, ud, x, xx, omega_i, zi)        
            Ai += A
            rhs_i += rhs

            A, rhs = update_W_bar_star(T, d, ud, x, xx, omega_f, zf)        
            Af += A
            rhs_f += rhs

            A, rhs = update_W_bar_star(T, d, ud, x, xx, 2*omega_p, zp)
            Ap += A
            rhs_p += rhs
        
            A, rhs = update_W_bar_star(T, d, ud, x, xx, omega_o, zo)        
            Ao += A
            rhs_o += rhs

            Sig_h_add += update_Sigma_H(T, d, h, hh_diags, zo, v)

            Sig_c_add += update_Sigma_C(T,d, c_0, cc, cc_off, c, zi, zf, zp)
            
            x_y, xx_y = get_Ex_Exx_No_U(T, d, h, hh)
            A, rhs = update_W_bar_y(T, d, yd, y, x_y, xx_y)
            Ay += A
            rhs_y += rhs
            
            sum_yy, sum_xyT,  sum_xx = update_Sigma_y(T,d, yd, y, x_y, xx_y)
            sum_yy_add += sum_yy
            sum_xyT_add += sum_xyT
            sum_xx_add += sum_xx
    ...
    W_bar_i = np.linalg.solve(Ai,rhs_i).reshape(d,d+ud+1)
    ...
    #Update Wf
    W_bar_f = np.linalg.solve(Af,rhs_f).reshape(d,d+ud+1)
    ...
    #Update Wp
    W_bar_p = np.linalg.solve(Ap,rhs_p).reshape(d,d+ud+1)
    ...
    #Update Wo
    W_bar_o = np.linalg.solve(Ao,rhs_o).reshape(d,d+ud+1)
    ...
    #Update Wy
    W_bar_y = rhs_y @ np.linalg.inv(Ay)
    ...
    #Update Sigma_y
    WxyT = W_bar_y @ sum_xyT_add
    Sigma_y = 1/(N-N_burn)*1/T*(sum_yy_add-WxyT-WxyT.T+
                                W_bar_y @ sum_xx_add @ W_bar_y.T)

    diff_list, Sigma_y_old = get_diff( Sigma_y, Sigma_y_old, diff_list)
    inv_covar_y = np.linalg.inv(Sigma_y)
    
    
    Ec_add = 1/(N-N_burn)*Ec_add
    Eh_add = 1/(N-N_burn)*Eh_add
    ...
    if k % 1 == 0:
        ...
        for j in range(0,T_new):
            if j ==0:
                ...
                c0 = Ec_add[-1,:,0].reshape(d) 
                h0 = Eh_add[-1,:,0].reshape(d)
...
for j in range(0,T_new):
    if j ==0:
        ...
        ct =  Ec_add[-1,:,0].reshape(d) 
        ht = Eh_add[-1,:,0].reshape(d)
