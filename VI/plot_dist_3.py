...
def get_likelihood(x,y,list, vars, c_minus, pi, pf, pp, po, var_c, var_h):
    sum_L  = 0
...
#L = get_likelihood(0,0,list, vars, c_minus, pi, pf, pp, po, var_c, var_h)
L = get_likelihood(X,Y,list, vars, c_minus, pi, pf, pp, po, var_c, var_h)
L = np.flip(L,axis=0)

#plt.scatter(c[0,:], h[0,:], 'g', label = 'LSTM')
...
for j in range(0,N):
    ...
    cnew,hnew = transform(min, max, size, cme, hm)
    plt.scatter(cnew,hnew, c='g')

plt.scatter(cnew,hnew, c='g', label='Samples') 
'''
