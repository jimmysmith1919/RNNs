...
L=-.9
U= .9
Wz,Uz,bz,Wz_mu_prior = update.init_weights(L,U, Sigma_theta, d, ud)
Wr,Ur,br,Wr_mu_prior  = update.init_weights(L,U, Sigma_theta, d, ud)
Wp,Up,bp,Wp_mu_prior  = update.init_weights(L,U, Sigma_theta, d, ud)

##TESTING###
Wz = np.random.uniform(0,2, size=(d,d))
Uz = np.random.uniform(0,2, size=(d,ud))
bz = np.random.uniform(0,2, size = (d,1))

Wr = np.random.uniform(0,2, size=(d,d))
Ur = np.random.uniform(0,2, size=(d,ud))
br = np.random.uniform(0,2, size = (d,1))

Wp = np.random.uniform(0,2, size=(d,d))
Up = np.random.uniform(0,2, size=(d,ud))
bp = np.random.uniform(0,2, size = (d,1))
...
for i in range(1,T+1):
    #TESTING
    h[i,:,0] = np.random.uniform(-.5,.5,3 )#np.random.multivariate_normal(h[i-1,:,0], np.diag(1/inv_var) )
...
for n in range(0,M):
    ...
    pv = expit(2*(Wp @ (z[0,:,0]*h0).reshape(d)+
    ...
    for i in range(1,T+1):
        ...
        if i != T:
             ...
             pv = expit(2*(Wp @ (z[1,:,0]*h[i,:,0]).reshape(d)+
